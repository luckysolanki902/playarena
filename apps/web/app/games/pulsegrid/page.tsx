...
export default function PulseGridLobby() {
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const data = await res.json();
      sfx.go();
      router.push(`/games/pulsegrid/${data.id}`);
    } catch {
  ...
  const playRandom = async () => {
    ...
    sfx.click();
    try {
      const listRes = await fetch(`${API}/rooms?game=pulsegrid`, {
      ...
      if (listRes.ok) {
        ...
        if (available.length > 0) {
          sfx.go();
          router.push(`/games/pulsegrid/${available[0].id}`);
          return;
        }
      }
      const res = await fetch(`${API}/rooms`, {
  ...
  const joinByCode = () => {
    ...
    sfx.click();
    router.push(`/games/pulsegrid/${encodeURIComponent(joinCode)}`);
  };
  ...
  return (
    <main className="min-h-screen flex flex-col relative stars-bg">
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="blob w-80 h-80 top-[-10%] right-[-5%] opacity-25" style={{ background: "rgba(34,211,238,0.4)" }} />
        ...
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity" onClick={() => sfx.click()}>
            <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm font-black"
              style={{ background: "var(--accent-primary)", color: "var(--bg-primary)" }}>S</div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-extrabold" style={{ color: "var(--text-primary)" }}>PulseGrid</span>
          </div>
          <div className="flex items-center gap-2 px-2.5 py-1 rounded-full"
            ...
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold"
              style={{ background: "#22d3ee", color: "var(--bg-primary)" }}>{session.username[0].toUpperCase()}</div>
          </div>
        </div>
      </nav>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>
              Pulse to claim territory. Dominate.
            </p>
          </div>

          <div className="flex flex-col gap-3">
            <button onClick={playRandom} disabled={matching} onMouseEnter={() => sfx.hover()}
              ...
              <div className="flex-1">
                ...
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>Battle for territory</p>
              </div>
              {!matching && (
            ...
            <button onClick={createRoom} disabled={creating} onMouseEnter={() => sfx.hover()}
              ...
              <div className="flex-1">
                <p className="text-sm font-bold" style={{ color: "var(--text-primary)" }}>
                  {creating ? "Creating arena..." : "Create Room"}
                </p>
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>Private battle with friends</p>
              </div>
              {!creating && (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="9 18 15 12 9 6"/>
            ...
            <button onClick={() => { setShowJoinInput(!showJoinInput); sfx.click(); }} onMouseEnter={() => sfx.hover()}
              className="game-card group flex items-center gap-4 p-4 rounded-2xl border text-left cursor-pointer"
              style={{ background: "var(--bg-card)", borderColor: showJoinInput ? "#22d3ee" : "var(--border-default)" }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0"
              ...
              <div className="flex-1">
                ...
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>Enter a room code</p>
              </div>
              <motion.svg animate={{ rotate: showJoinInput ? 90 : 0 }}
