...
export default function VoidfallLobbyPage() {
  ...
  async function handleQuickMatch() {
    ...
    try {
      const listRes = await fetch(`${API}/rooms?game=voidfall`, { headers: { Authorization: `Bearer ${token}` } });
      if (listRes.ok) {
        ...
        if (available.length > 0) { sfx.go(); router.push(`/games/voidfall/${available[0].id}`); return; }
      }
      const res = await fetch(`${API}/rooms`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ game: "voidfall", visibility: "public", maxPlayers: 6 }) });
      if (!res.ok) throw new Error("Failed");
      const data = await res.json();
      sfx.go(); router.push(`/games/voidfall/${data.id}`);
  ...
  async function handleCreateRoom() {
    ...
    try {
      const res = await fetch(`${API}/rooms`, {
        ...
        body: JSON.stringify({ game: "voidfall", visibility: "private", maxPlayers: 6 }),
      });
      if (!res.ok) throw new Error("Failed to create room");
      const data = await res.json();
      sfx.click();
      router.push(`/games/voidfall/${data.id}`);
  ...
  async function handleJoinByCode() {
    ...
    try {
      ...
      sfx.click();
      router.push(`/games/voidfall/${room.id}`);
    ...
    <div className="min-h-screen flex flex-col relative stars-bg">
      ...
      <header className="relative z-10 w-full flex items-center justify-between px-4 sm:px-6 py-3 border-b" style={{ borderColor: "var(--border-subtle)" }}>
        ...
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md flex items-center justify-center text-xs font-black" style={{ background: `${GAME_COLOR}22`, color: GAME_COLOR }}>V</div>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center gap-6 px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-4 text-center">
          ...
          <h1 className="text-2xl font-extrabold" style={{ color: "var(--text-primary)" }}>Voidfall</h1>
          <p className="text-sm max-w-xs" style={{ color: "var(--text-muted)" }}>
            Dodge the shrinking void zone. Stay inside the safe area!
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          ...
          <p>Move with arrow keys or WASD. The safe zone shrinks over time. Last one inside wins!</p>
        </motion.div>
      </main>
    </div>
  );
