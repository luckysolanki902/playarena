...
const GAME_COLOR = "#4ecdc4";
const GAME_LETTER = "W";
const GAME_DESC = "Guess the hidden word in 6 tries. Race against others!";
const HOW_TO_PLAY = "Each guess reveals clues: green = correct, yellow = wrong spot, gray = not in word.";
const MAX_PLAYERS = 6;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center stars-bg">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center text-3xl font-black" style={{ background: `${GAME_COLOR}22`, color: GAME_COLOR }}>{GAME_LETTER}</div>
          <p className="text-sm font-semibold mb-4" style={{ color: "var(--text-secondary)" }}>Pick a name first to play!</p>
          <Link href="/" className="btn-game inline-block px-6 py-3 rounded-2xl text-sm font-bold" style={{ background: GAME_COLOR, color: "var(--bg-primary)" }}>Go to home</Link>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col relative stars-bg">
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center gap-6 px-4 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col gap-3 w-full max-w-xs">
          <Link href="/games/wordle/solo" onClick={() => sfx.click()} className="btn-game px-6 py-4 rounded-2xl font-bold text-base text-center cursor-pointer" style={{ background: "var(--bg-card)", border: "1px solid var(--border-default)", color: "var(--text-primary)" }}>🎯 Solo Practice</Link>
